...
- `rustdoc_json::find_lib_json_path()` may run a cached full `cargo metadata` call to map Cargo package names to Rust lib/proc-macro target names before accepting a cached rustdoc JSON path. This is required for crates whose `[lib] name` differs from the package name, prevents stale package-derived JSON files from shadowing the real lib-target JSON, and is cached per manifest path for the process. Version-qualified specs (`name@version`) are resolved against version-qualified metadata entries; unversioned multi-version packages are treated as ambiguous rather than accepting a package-stem cache hit.
