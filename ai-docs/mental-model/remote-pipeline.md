...
- `generate_rustdoc_json()` auto-retries with a version-qualified spec when cargo reports "is ambiguous", but only in the non-verbose (captured output) path. In verbose mode (`Stdio::inherit()`), cargo's stderr is not captured, so the ambiguity error is never detected and the call bails immediately with the raw cargo error. Multi-version crates will therefore fail under `--verbose` unless a version-qualified spec was passed in from the start.
