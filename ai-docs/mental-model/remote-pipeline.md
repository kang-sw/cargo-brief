...
- `load_remote_feature_graph(spec)` in `remote.rs` calls `fetch_crates_io_api(name)` to fetch the crates.io metadata JSON, then extracts the `features` map for the resolved version. It merges `features2` into `features` so that `dep:`-prefixed weak-dependency aliases are included. Returns `Ok(None)` on any network/parse failure (graceful offline fallback) — callers must handle `None` as "feature graph unavailable, skip pre-validation". The crates.io API response is already cached by `fetch_crates_io_api` at `cache_dir()/versions/{name}.json` (24h TTL), so `load_remote_feature_graph` piggybacks on that cache.
...
- `generate_rustdoc_json(..., use_cache=true)` skips `cargo rustdoc` if the correct `.json` file already exists in `target/doc/`. Rustdoc JSON lookup validates the Rust lib/proc-macro target name from cargo metadata when available, so stale package-derived JSON files do not shadow crates whose `[lib] name` differs from the package name. Remote pipelines always set `use_cache=true`. Local pipelines set `use_cache` based on workspace membership: true for non-member deps (e.g. `bevy` from a game project), false for workspace members (source may have changed).
...
- Rustdoc JSON filename resolution caches the manifest-level package → lib/proc-macro target-name map in process. File existence is not cached, because batch generation may create the JSON after an earlier miss in the same operation.
...
- **Add feature flag support for sub-crates**: `-F`/`--features` on `BriefDirect` is wired into `RemoteOpts.features`, passed to `resolve_workspace`, and pre-validated against the remote `FeatureGraph` before `rustdoc` is invoked. To propagate features to cross-crate deps, modify `write_workspace_files()` in `remote.rs` to accept per-dep feature lists.
