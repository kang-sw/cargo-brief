...
- `run_daemon()` guarantees: (1) writes PID file before binding socket; (2) LSP initialize completes before the UDS socket is bound and accepting — clients that connect will always find an initialized (or failed) ra instance, never one mid-handshake; (3) idle timeout defaults to 600s, overridable via `CARGO_BRIEF_LSP_TIMEOUT` env var; (4) cleans up socket + PID files on exit (both normal and idle timeout; but NOT on panic or kill -9).
