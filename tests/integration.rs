...
fn test_trait_impl_is_one_liner() {
    ...
    // Simple trait impl (no associated types) should be a one-liner
    assert!(
        output.contains("impl MyTrait for PubStruct { .. }"),
        "trait impl should be one-liner: got:\n{output}"
    );
    // Should NOT contain the expanded method body (multi-line block)
    assert!(
        !output.contains("impl MyTrait for PubStruct {\n"),
        "trait impl should not have expanded braces"
    );
}
